...
struct MessagesListView: View {
    ...
    // Define the colors.  These should be in your AppColors
    let forestGreen = Color(red: 0.18, green: 0.55, blue: 0.34) // Example: Forest Green
    let pearlWhite = Color(red: 0.97, green: 0.96, blue: 0.94)  // Example: Pearl White
    ...
    var body: some View {
        Group {
            if isLoading {
                ...
                List {
                    ForEach(conversations) { conversation in
                        NavigationLink(destination: ChatDetailView(partnerID: conversation.partnerID, partnerName: conversation.partnerName, forestGreen: forestGreen, pearlWhite: pearlWhite)) {
                            ConversationRow(conversation: conversation, forestGreen: forestGreen, pearlWhite: pearlWhite)
                        }
                    }
                }
                .listStyle(InsetGroupedListStyle())
...
struct ChatDetailView: View {
    ...
    @State private var messages: [MessageViewModel] = []
     ...
     // Use the colors passed in.
    let forestGreen: Color
    let pearlWhite: Color
    ...
    var body: some View {
        VStack {
            ScrollView {
                LazyVStack {
                    ForEach(messages) { message in
                        MessageBubble(message: message, forestGreen: forestGreen, pearlWhite: pearlWhite)
                    }
                }
                .padding()
            }
            ...
            VStack(alignment: .trailing, spacing: 4) {
                ...
                if conversation.unreadCount > 0 {
                    Text("\(conversation.unreadCount)")
                        ...
                        .foregroundColor(.white)
                        .padding(6)
                        .background(Circle().fill(forestGreen))
                }
            }
        }
        .padding(.vertical, 8)
...
struct MessageBubble: View {
    ...
    var body: some View {
        HStack {
            ...
            Text(message.text)
                .padding(10)
                .background(message.isSentByMe ? forestGreen : pearlWhite)
                .foregroundColor(message.isSentByMe ? .white : .primary)
                .cornerRadius(16)
